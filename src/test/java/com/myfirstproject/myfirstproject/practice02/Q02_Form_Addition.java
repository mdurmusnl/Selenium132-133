...
//        Fill the form
        Thread.sleep(1000);
        driver.findElement(By.name("first_name")).sendKeys(Faker.instance().name().firstName());//this way is also possible
        Thread.sleep(1000);
        driver.findElement(By.name("last_name")).sendKeys(faker.name().lastName());
        Thread.sleep(1000);
        driver.findElement(By.name("business_name")).sendKeys(faker.name().title());
        Thread.sleep(1000);
        driver.findElement(By.name("email")).sendKeys(faker.internet().emailAddress());
        Thread.sleep(1000);

        //Locate number1   valueof() return wrapper Integer  parsInt() returns primitive int
...
//        Assert that the form has been sent
  WebElement circle=driver.findElement(By.id("colored"));
        Thread.sleep(1000);
